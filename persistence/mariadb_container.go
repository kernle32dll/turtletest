...
// ShutdownContainer shuts down the container.
...
func (s *MariaDBContainer) removeDatabase(t testing.TB, ctx context.Context) {
	...
	if _, err := db.ExecContext(ctx, "DROP DATABASE `"+s.databaseName+"`"); err != nil {
		// Not breaking, just informal
		t.Logf("Failed to delete database %q: %s", s.databaseName, err)
	}
}

func (s *MariaDBContainer) ensureDatabase(t testing.TB, ctx context.Context) *sqlx.DB {
	t.Helper()
